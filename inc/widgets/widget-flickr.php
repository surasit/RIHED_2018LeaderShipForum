...
class WPlookflickr extends WP_Widget {
	...
	public function __construct() {
		parent::__construct(
	 		'WPlookflickr',
			__( 'WPlook Flickr', 'event-wpl' ),
			array( 'description' => __( 'A widget for displaying Flickr images', 'event-wpl' ), )
		);
	}
	...
	function form($instance) {
		if ( $instance ) {
			...
			$title = __( 'My Flickr', 'event-wpl' );
		} 

		if ( $instance ) {
			$flickrrss = esc_attr( $instance[ 'flickrrss' ] );
		} else {
			$flickrrss = __( 'insert flickr RSS', 'event-wpl' );
		} 

		if ( $instance ) {
			$thumbs = esc_attr( $instance[ 'thumbs' ] );
		} else {
			$thumbs = __( '6', 'event-wpl' );
		} ?>
		
		<!-- Title-->
		<p>
			<label for="<?php echo $this->get_field_id('title'); ?>">
				<?php _e('Title:', 'event-wpl'); ?>
			</label>
		...
		<p style="font-size: 10px; color: #999; margin: -10px 0 0 0px; padding: 0px;">
			<?php _e('Insert your Flickr RSS.', 'event-wpl'); ?>
		</p>
		</p>
		<!-- Number of thumbs -->
		<p>
			<label for="<?php echo $this->get_field_id('thumbs'); ?>">
				<?php _e('Number of thumbs:', 'event-wpl'); ?>
			</label>
		...
		<p style="font-size: 10px; color: #999; margin: -10px 0 0 0px; padding: 0px;">
			<?php _e('Number of thumbnails you want to display.', 'event-wpl'); ?>
		</p>
		</p>
	...
	function widget($args, $instance) {
			...
			require_once(ABSPATH . 'wp-includes/class-simplepie.php');	
				...
				$feed = new SimplePie();
				if ($flickrrss == '') {
					$feed->set_feed_url('http://api.flickr.com/services/feeds/photos_public.gne?tags=ong&lang=en-us&format=rss_200');
				} else {
			...
			foreach ($feed->get_items(0,$thumbs) as $item) {
				...
				if ($enclosure = $item->get_enclosure()) {
					...
					echo '<a href="' . $full_url . '" target="_blank" title="' . $enclosure->get_title() . '"><img width="100" height="75" src="' . $thumb_url . '" /></a>'."\n";
				}
			?>
